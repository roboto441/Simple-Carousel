...
function loadCss(container, customBtn, customDots, animationType) {    
    ...
    $('head').append('<style>' + container + ' { transform: translate3d(0,0,0); -webkit-transform: translate3d(0,0,0); box-sizing: content-box; background-color: #000000; overflow: hidden; position: relative; -webkit-touch-callout: none; -webkit-user-select: none; -khtml-user-select: none; -moz-user-select: none; -ms-user-select: none; user-select: none;}' + container + ' img { display: none; width: 100%; } '  + container + ' #last,' + container + ' #current,' + container + ' #next { height: 100%; width: 100%; position: absolute; top: 0; }' + container + ' #last img,' + container + ' #current img,' + container + ' #next img { display: block; position: relative; top: 50%; transform: translateY(-50%);  -webkit-transform: translateY(-50%); }' + container + ' #last { left: -100% }' + container + ' #current { left: 0 }' + container + ' #next { left: 100% }' + buttons + dots + fade + '</style>');
}
